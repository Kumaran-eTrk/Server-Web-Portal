...
import { UpdateApproveSoftwarePopup } from "../softwareconfirmationpopup/UpdateApproveSoftwarePopup";
...
const RejectSoftwareTab = () => {
  const [updateSoftwareReject, setUpdateSoftwareReject] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  const styles = useStyles();
  const inputRef = React.useRef<HTMLInputElement | null>(null);

  useEffect(() => {
    const fetchRejectedSoftwareData = async () => {
      try {
        const response = await api.get(`/softwares/rejectedstatus`);
        ...
        setUpdateSoftwareReject(sortedData);
      } catch (error) {
        console.error("Error fetching Extension data:", error);
      }
    };

    fetchRejectedSoftwareData();
  }, []);
  ...
  const filteredSoftware = updateSoftwareReject.filter((item: Item) =>
  ...
  const columns: TableColumnDefinition<Item>[] = [
    ...
    createTableColumn<Item>({
      ...
      renderCell: (item) => {
        return (
          <TableCellLayout>
            <UpdateApproveSoftwarePopup
              setUpdateSoftwareReject={setUpdateSoftwareReject}
              softwareId={item.id}
            />
          </TableCellLayout>
        );
  ...
  return (
    <div role="tabpanel" aria-labelledby="Rejectd">
      <div
        style={{
          display: "flex",
...
export default memo(RejectSoftwareTab);
