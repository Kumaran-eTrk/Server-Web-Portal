...
import { UpdateApprovePopup } from './confirmationpopup/UpdateApprovePopup';
...
const RejectedTab: React.FC<IListWidgetProps> = () => {
  const [updateReject, setUpdateReject]:any = useState([]);
  ...
  const inputRef = React.useRef<HTMLInputElement | null>(null);
    ...
    const filteredExtension = updateReject.filter((item: Item) =>
    ...
    const columns: TableColumnDefinition<Item>[] = [
      ...
      createTableColumn<Item>({
        ...
        renderCell: (item) =>{ 
        return <TableCellLayout >
          <UpdateApprovePopup setUpdateReject ={setUpdateReject} extensionId={item.id}/>
        </TableCellLayout>} 
      }),   
    ];
  
    return (
      <div role="tabpanel" aria-labelledby="Rejected">
...
export default memo(RejectedTab);
